...
def parse_config_entry(entry, globalconfig, work_dir, authority_tos_agreement):
        ...
        try:
            ...
            for domain in config['domainlist']:
                if any(ord(c) >= 128 for c in domain):
                    # Append IDNA domain name for a unicode domain
    ...
    for domain in config['domainlist']:
        ...
        specificcfgs = [x for x in handlerconfigs
                        if 'domain' in x and x['domain'] == config.get('domaintranslation', {}).get(domain, domain)]
